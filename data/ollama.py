...
# Карта ISO-кодов → название языка по-русски (так формулируем промпт,
# потому что модель qwen2.5 лучше понимает по-русски). Для незнакомых
# кодов используем сам код в верхнем регистре — модель догадается.
_LANG_NAMES = {
    "ru": "русский",
    "en": "английский",
    "es": "испанский",
...
def translate(text: str, target_lang: str, model: str = None) -> str:
    """Перевести `text` на `target_lang` (ISO-код, например 'en').
    Возвращает чистый перевод без префиксов («Перевод: …»). Если текст
    уже на целевом языке — модель обычно отдаёт его почти без изменений,
    это ок.

    Бросает RuntimeError если Ollama недоступна."""
    if not text or not text.strip():
        return ""
    lang_name = _LANG_NAMES.get(target_lang.lower(), target_lang.upper())
    # Промпт намеренно строгий: только перевод, никаких комментариев,
    # без рамок ```code```. qwen2.5:3b иногда добавляет «Вот перевод:» —
    # сдерживаем это явной инструкцией и постобработкой.
    prompt = (
        f"Переведи следующий текст на {lang_name} язык. "
        f"Сохрани смысл, эмодзи, форматирование. "
        f"В ответе только сам перевод — без вводных слов, без кавычек "
        f"вокруг всего ответа, без пояснений.\n\n"
        f"ТЕКСТ:\n{text}\n\nПЕРЕВОД:"
    )
    raw = generate(prompt, model=model, temperature=0.2)
    # Срезаем типичный мусор от мелких моделей.
    raw = raw.strip()
    for prefix in ("Перевод:", "ПЕРЕВОД:", "Translation:", "Вот перевод:"):
        if raw.startswith(prefix):
            raw = raw[len(prefix):].strip()
    # Снимаем парные внешние кавычки, если LLM обернула весь ответ.
